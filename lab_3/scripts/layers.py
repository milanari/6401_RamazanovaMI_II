...
def affine_backward(dout, cache):
    ...
    dx = dx_reshaped.reshape(original_shape)
...
def batchnorm_forward(x, gamma, beta, bn_param):
    ...
    if mode == "train":
        ...
        x_norm = x_centered / np.sqrt(sample_var + eps)
        ...
        cache = (x, x_centered, x_norm, sample_mean, sample_var, gamma, eps)
...
def batchnorm_backward(dout, cache):
    ...
    x, x_centered, x_norm, sample_mean, sample_var, gamma, eps = cache
    N, D = dout.shape
    
    dbeta = np.sum(dout, axis=0)           # dout/dbeta = 1
    dgamma = np.sum(dout * x_norm, axis=0) # dout/dgamma = x_norm
    
    dx_norm = dout * gamma
    
    s = np.sqrt(sample_var + eps)
    dx_centered = dx_norm / s

    dvar = np.sum(dx_norm * x_centered * (-0.5) * (sample_var + eps)**(-1.5), axis=0)

    dmean = -np.sum(dx_centered, axis=0) + dvar * np.sum(-2 * x_centered, axis=0) / N
